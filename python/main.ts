...
namespace cvFindContours{
    ...
    //% block="寻找[IMG]轮廓点 返回([CONTOURS],[HIERARCHY])" blockType="command"
    //% IMG.shadow="string" IMG.defl="img"
    //% CONTOURS.shadow="string" CONTOURS.defl="contours"
    //% HIERARCHY.shadow="string" HIERARCHY.defl="hierarchy"
    export function findContours(parameter: any, block: any) {
        ...
        Generator.addImport(`import cv2\nimport numpy as np`)
        Generator.addCode(`${contours},${hierarchy} = cv2.findContours(${img},cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)`)
    ...
    //% block="获取点坐标[OBJ]角点坐标 返回(X[X],Y[Y],宽[W],高[H])" blockType="command"
    //% OBJ.shadow="string" OBJ.defl="approx"
    ...
    export function boundingRect(parameter: any, block: any) {
        ...
        Generator.addImport(`import cv2\nimport numpy as np`)
        Generator.addCode(`${x}, ${y}, ${w}, ${h} = cv2.boundingRect(${obj})`)
 
    }



    //% block="计算[OBJ]轮廓角数量" blockType="reporter"
    //% OBJ.shadow="string" OBJ.defl="approx"
    ...
    export function drawContours(parameter: any, block: any) {
        ...
        Generator.addImport(`import cv2\nimport numpy as np`)
        Generator.addCode(`cv2.drawContours(${img}, ${obj}, -1, (${r},${g},${b}), ${thick})`)
    ...
    export function rectangle(parameter: any, block: any) {
        ...
        Generator.addImport(`import cv2\nimport numpy as np`)
        Generator.addCode(`cv2.rectangle(${img},(${x},${y}),(${x}+${w},${y}+${h}),(${r},${g},${b}),${thick})`)
    ...
    export function putText(parameter: any, block: any) {
        ...
        Generator.addImport(`import cv2\nimport numpy as np`)
        Generator.addCode(`cv2.putText(${img},${txt},(${x},${y}),cv2.FONT_HERSHEY_COMPLEX,${size},(${r},${g},${b}),1)`)
